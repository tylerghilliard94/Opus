...
export default function Favorites(props) {
    ...
    return (
        <>
            ...
            {like.userProfileId == sessionStorage.userProfileId && like.postId == artPost.id ? <Col className="DisLikeButton" sm={3}><Button onClick={handleRemoveLike}>Dislike</Button></Col> : <Col className="LikeButton" sm={3}><Button className="SelectButton" onClick={handleLike}>Like</Button></Col>}<Col className="LikeCounter" sm={1}><h5>{artPost.likes}</h5></Col>
        </>
    )
